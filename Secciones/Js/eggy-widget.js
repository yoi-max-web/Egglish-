/* ============================================================
  ...
  reutiliza el motor local de Transformers.js para responder.
   ============================================================ */
...
} from '/Secciones/Js/webllm-engine.js';
...
function injectStyles() {
  ...
  .eggy-fab-boot.is-hidden { display: none; }
  .eggy-fab-boot img { width: 62px; height: 62px; object-fit: contain; }
  .eggy-fab-boot h2 { margin: 0; color: var(--text); font-size: 1.05rem; font-weight: 900; }
  .eggy-fab-boot p { margin: 0; color: var(--mid); font-size: .8rem; font-weight: 700; }
  .eggy-fab-boot__track {
    width: min(220px, 90%);
    height: 10px;
    overflow: hidden;
    border-radius: 999px;
  ...
  .eggy-fab-boot.is-error .eggy-fab-boot__retry { display: block; }
...
function buildMarkup() {
  ...
  root.innerHTML = `
    ...
    <section class="eggy-fab-panel" id="eggy-fab-panel" aria-label="Chat con Eggy IA" aria-hidden="true">
      ...
      <div class="eggy-fab-boot" id="eggy-fab-boot" aria-live="polite">
        <img src="${CHICK_URL}" alt="Eggy"/>
        <h2>Preparando a Eggy para ti</h2>
        <p id="eggy-fab-boot-text">Preparando tu asistente...</p>
        <div class="eggy-fab-boot__track" aria-hidden="true">
          <div class="eggy-fab-boot__fill" id="eggy-fab-boot-fill"></div>
        </div>
        <button class="eggy-fab-boot__retry" id="eggy-fab-boot-retry" type="button">Intentar de nuevo</button>
      </div>
      ...
      <section class="eggy-fab-messages" id="eggy-fab-messages" aria-live="polite">
        <div class="eggy-fab-welcome" id="eggy-fab-welcome">
          ...
          <div class="eggy-fab-chips">
            <button class="eggy-fab-chip" data-suggestion="¿Cuál es la diferencia entre 'a few' y 'a little'?">🥚 a few vs a little</button>
...
function init() {
  ...
  const boot        = document.getElementById('eggy-fab-boot');
  const bootText    = document.getElementById('eggy-fab-boot-text');
  const bootFill    = document.getElementById('eggy-fab-boot-fill');
  const bootRetry   = document.getElementById('eggy-fab-boot-retry');
  const chipBtns    = messages.querySelectorAll('.eggy-fab-chip');
  const status      = document.querySelector('.eggy-fab-panel__status');

  let isOpen = false;
  let isThinking = false;
  let hasInjectedWelcome = false;
  let engineReady = false;
  let preparingEngine = null;
  ...
  function createMessageNode(sender, text) {
    ...
    content.className = 'eggy-fab-msg__content';
  ...
  function updateSendBtn() {
    ...
    sendBtn.disabled = !hasText || isThinking || !engineReady;
  }

  async function prepareEngine() {
    if (engineReady) return true;
    if (preparingEngine) return preparingEngine;

    boot?.classList.remove('is-hidden', 'is-error');
    if (bootText) bootText.textContent = 'Preparando tu asistente...';
    if (bootFill) bootFill.style.width = '0%';
    textarea.disabled = true;
    sendBtn.disabled = true;
    if (status) status.textContent = 'Preparando...';

    preparingEngine = initEngine(({ progress }) => {
      const percent = Math.round(Math.max(0, Math.min(1, progress)) * 100);
      if (bootFill) bootFill.style.width = `${percent}%`;
      if (bootText) bootText.textContent = percent >= 100
        ? 'Casi listo...'
      ...
      console.error('[Eggy Widget] No se pudo preparar el asistente:', error);
      if (bootText) bootText.textContent = getEngineErrorMessage(error);
      boot?.classList.add('is-error');
      if (status) status.textContent = 'No disponible';
      return false;
    }).finally(() => {
      preparingEngine = null;
    });
  ...
  async function sendMessage(overrideText) {
    ...
    async function generateReply() {
      ...
      try {
        ...
        await preguntarEggyStream(
          ...
          (_delta, fullText) => {
            if (!replyNode) {
              hideThinking();
              replyNode = createMessageNode('eggy', '');
              replyContent = replyNode.querySelector('.eggy-fab-msg__content');
              messages.insertBefore(replyNode, thinking);
            }
            reply = fullText;
            replyContent.innerHTML = parseMarkdown(reply);
            scrollToBottom();
          },
      ...
      if (!reply.trim()) {
        throw new Error('El modelo no produjo texto. Inténtalo de nuevo.');
      }
      if (!replyNode) appendMessage('eggy', reply);
      return reply;
    }
    ...
    try {
      ...
      try {
        ...
        if (!isRecoverableEngineError(error)) throw error;
        engineReady = false;
        resetEngineForRetry();
        await prepareEngine();
      ...
      console.error('[Eggy Widget] Error al generar una respuesta local:', error);
      appendMessage('eggy', `❌ ${getEngineErrorMessage(error)}`);
    } finally {
      hideThinking();
      if (status && engineReady) status.textContent = 'En línea';
    }
  }
  ...
  bootRetry.addEventListener('click', prepareEngine);
